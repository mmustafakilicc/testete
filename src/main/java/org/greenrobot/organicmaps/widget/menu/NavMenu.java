package org.greenrobot.organicmaps.widget.menu;
...
public class NavMenu
{
  ...
  {
    mTts.setImageDrawable(TtsPlayer.isEnabled() ? Graphics.tint(mActivity, R.drawable.mn_org_map_ic_voice_on,
        androidx.appcompat.R.attr.colorAccent)
        : Graphics.tint(mActivity, R.drawable.mn_org_map_ic_voice_off));
  }
  ...
  private void updateSpeedView(@NonNull RoutingInfo info)
  {
    ...
    if (info.speedLimitMps > 0.0 && last.getSpeed() > info.speedLimitMps)
    {
      if (info.isSpeedCamLimitExceeded())
        mSpeedValue.setTextColor(ContextCompat.getColor(mActivity, R.color.mn_org_map_white_primary));
      else
        mSpeedValue.setTextColor(ContextCompat.getColor(mActivity, R.color.mn_org_map_base_red));
    }
    else
