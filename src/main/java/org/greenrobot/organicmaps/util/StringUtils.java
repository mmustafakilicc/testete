...
public class StringUtils
{
  ...
  public static String getFileSizeString(@NonNull Context context, long size)
  {
    if (size < Constants.GB)
    {
      ...
      return formatUsingUsLocale("%1$d %2$s", value, OrganicMapsModule.get().getApplication().getString(R.string.mb));
    }

    float value = ((float) size / Constants.GB);
    return formatUsingSystemLocale("%1$.1f %2$s", value, OrganicMapsModule.get().getApplication().getString(R.string.gb));
  }
