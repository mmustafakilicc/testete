...
public final class OsmOAuth {
    ...
    public static boolean isAuthorized(@NonNull Context context) {
        return SharedPrefUtils.INSTANCE.isAuthorized(context);
    }

    public static boolean containsOAuth1Credentials(@NonNull Context context) {
        return SharedPrefUtils.INSTANCE.containsOAuth1Credentials(context);
    }

    public static void clearOAuth1Credentials(@NonNull Context context) {
        SharedPrefUtils.INSTANCE.clearOAuth1Credentials(context);
    }

    public static String getAuthToken(@NonNull Context context) {
        return SharedPrefUtils.INSTANCE.getAuthToken(context);
    }

    public static String getUsername(@NonNull Context context) {
        return SharedPrefUtils.INSTANCE.getUserName(context);
    }
    ...
    public static void setAuthorization(@NonNull Context context, String oauthToken, String username) {
        SharedPrefUtils.INSTANCE.setAuthorization(context, oauthToken, username);
    }

    public static void clearAuthorization(@NonNull Context context) {
        SharedPrefUtils.INSTANCE.clearAuthorization(context);
    }
    ...
    public static int getOsmChangesetsCount(@NonNull Context context, @NonNull FragmentManager fm) {
        ...
        if (editsCount[0] < 0)
            return SharedPrefUtils.INSTANCE.getOsmChangesetsCount(context);

        SharedPrefUtils.INSTANCE.setOsmChangesetsCount(context, editsCount[0]);
        return editsCount[0];
    }
}
